...
CREATE TABLE IF NOT EXISTS opportunities (
 id INT AUTO_INCREMENT PRIMARY KEY,organization_id INT NOT NULL,title VARCHAR(255) NOT NULL,category VARCHAR(100) NOT NULL,location VARCHAR(255) NOT NULL,time_commitment VARCHAR(50) NOT NULL,spots INT NOT NULL,start_date DATE NOT NULL,description TEXT NOT NULL,urgent TINYINT(1) DEFAULT 0,status ENUM('active','closed','draft') DEFAULT 'active',created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,FOREIGN KEY(organization_id) REFERENCES organizations(id) ON DELETE CASCADE,INDEX(category),INDEX(location),INDEX(status)
) ENGINE=InnoDB;
